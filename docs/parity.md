...
Rows describe the evidence available for their semantic group. Later groups
explicitly supersede earlier implementation limits; the feature matrix records
the current remaining scope.
...
| A78 | Catalogue HTTP errors use the source consumer boundaries under A45/A77. Wire comparisons use owned HTTP/1 endpoints and reason bytes accepted by the native parser. | Probe invalid status lines, additional transport/read exception text, HTTP/2, TLS/proxy transformations, live servers and native Windows. Compare the vendored Hyper tree and exact patch with the published 1.10.1 crate. | 888 source HTTP-failure comparisons, the expanded 927 redirect-message comparisons, eight socket cases and sixteen CLI scenarios cover body reads, strict UTF-8, reason text and publication order. Hyper's validated obs-text reason bytes are retained by a documented local patch. Complete transport/framing/exception-text parity remains open. |
| A79 | Discovery response/cache decoding targets CPython 3.13.15 with its default integer limit. Unicode casing uses Rust's scalar tables, with surrogate code points separating casing contexts. | Change the interpreter's integer/recursion limits or Unicode version; probe native Windows, non-UTF-8 cache paths, locale-dependent cache decoding, resource exhaustion, malformed diagnostics and GraphQL/cache model JSON. | 276 raw-document source comparisons cover strict UTF-8 text, nonfinite values, escaped surrogates, selected-name code points, request order and exact candidate publication. Four CLI tests cover 24 invocations, including cache reuse and earlier metadata-directory probes. Discovery no longer requires serde-representable values; GraphQL/model JSON, runtime-dependent limits and exact malformed diagnostics remain open. |
| A80 | Catalogue GraphQL/cache JSON targets CPython 3.13.15 and the actual Pydantic models under A45. Model strings retain Python code points; native filesystem observations use owned macOS fixtures. | Probe other interpreter/Pydantic versions, modified integer/recursion limits, locale-dependent cache decoding, surrogate-containing cached archive URLs or file URLs, native Windows path normalization, filesystems accepting raw-byte names, resource failures and aggregate model diagnostics. | 882 raw-model/cache-text comparisons, 116 raw GraphQL-envelope comparisons, 10,240 surrogate path encodings and two native existence probes supplement the existing model/query/acquisition corpora. Five CLI tests cover nine scenarios and thirteen macOS invocations. Model strings survive publication and filtering; cache-key, filename and HTTP-URL conversion occur at their consumers. Downstream archive-index URL representation, file-URL surrogates, runtime/platform limits and exhaustive diagnostics remain open. |
| A81 | Shared archive-index URL ordering and consumer serialization target the pinned HCLI source and A45 runtime. Plain stdout comparisons use UTF-8 with Unix surrogateescape. | Probe surrogate file-URL transport, native Windows consoles, redirected output with alternate encodings or PYTHONIOENCODING, Rich terminal formatting, resource exhaustion and full exception text. | 2,053 index/snapshot/model-input comparisons and 2,050 search JSON/stdout codec comparisons cover every unpaired surrogate. Three CLI tests cover seven scenarios and twenty-three macOS invocations. Cached URLs survive acquisition, filtering and sorting; snapshot serialization remains strict while search JSON escapes Python strings. Other transport, codec and diagnostic profiles remain open. |
| A82 | Catalogue file requests use CPython 3.13.15 urllib and the A45 HCLI runtime. Native reads use owned macOS fixtures, UTF-8/surrogateescape and the observed ASCII/IPv4 resolver profile; Windows path conversion is compared as code points. | Probe Unicode hostname IDNA/nameprep, alternate DNS/address ordering, resolver changes after hostname caching, alternate filesystem codecs, native Windows file opens, concurrent replacement, response-header/read failures and full exception diagnostics. | 12,040 request/path comparisons and 72 actual source file reads compare payloads, exception categories, failure phases, attempt counts and wait schedules. Four CLI tests cover fifteen scenarios/invocations. Catalogue downloads now preserve query/control text and trailing separators, enforce local-host rules, and retry file opens before cache publication. Shared repository-fetch surrogate URLs, header-stage timestamp/MIME checks and the listed runtime/codec/diagnostic profiles remain open. |
| A83 | File-header validation targets the pinned HCLI handler and CPython 3.13.15 runtime. Calendar conversion uses the host C runtime; macOS ARM64 stat floats use the fused arithmetic observed in A45's Clang build. MIME mappings are unmodified. | Probe other CPython compiler contraction settings, native Windows/CRT behavior, alternate libc calendars, timezone/leap-second data, MIME database initialization failures, timestamp clamping and concurrent stat/open changes. Recheck other stat-float consumers independently. | 16,747 timestamp, 8,552 MIME-selector, 72 controlled handler/retry and 48 native metadata comparisons cover represented header failures and ordering. One CLI test covers assets and sources using the filesystem-retained timestamp. The macOS volume clamps year 10000 into range, so invalid-date ordering uses controlled stat results. Other runtime/resource/diagnostic profiles and complete project parity remain open. |
...
| GitHub catalogue | Extra/ignored lists, discovery including forks, complete release/tag/commit/asset models, distribution/source archives, date/type/size filters and source identity checks. A69–A74 cover nested retries, ten-repository GraphQL batches, partial results, validation/publication order, global asset/source phases, duplicate retention, cache-before-size behavior, expiry/error rules and list/search selection. A75 uses upstream shared cache paths and JSON text, validates archive cache keys and publishes through direct file writes. A76 aligns iterable candidate-cache roots and scalar search-name failure timing. A77 adds urllib redirect methods/headers, lexical targets, loop limits and history across retries. A78 preserves HTTP reason phrases, redirect rejection messages and GraphQL error-body/UTF-8 behavior. A79 preserves discovery Python JSON values through pagination, filtering and publication. A80 extends strict UTF-8/Python JSON decoding to GraphQL and metadata caches, preserves model strings, formats Python error values and defers archive-key/filename conversion to acquisition. A81 retains Python URL strings through shared archive indexing and applies snapshot/search serialization at the consumer. A82 separates catalogue urllib file requests from ordinary plugin file fetches, preserving query/path text, host checks and retry boundaries. Metadata expires after 86,400 seconds; archive bytes persist across tokens. | Shared repository-fetch surrogate file URLs, Unicode hostname IDNA/DNS profiles, alternate stdout codecs, runtime-dependent JSON limits/casing, exhaustive model coercion/diagnostics, concurrent/partial I/O and unusual filesystem aliases, FTP execution, nonstandard URL wire normalization, additional ValueError/network-error mappings, read/transport-error diagnostics, live private repositories and native Windows remain unverified or incomplete. |
| Catalogue file response headers | A83 validates the stat-derived timestamp and MIME selector before host resolution/open. Float-to-time conversion retains microsecond rounding, native time_t/calendar failures and datetime year validation. Skip, retry and fatal categories follow the source header stage. | Alternate CPython compiler arithmetic, native Windows/CRT and libc profiles, MIME database initialization failures, concurrent replacement and exact OS-error text remain unverified. |
...
The latest uninterrupted serial all-target run passed 298 unit tests and 484
integration tests on macOS: 782 passed, no failures. A83 records this validation;
...
Catalogue model conversion uses A73's complete upstream model projections,
shared-integer comparisons and CLI validation/cache migration regressions.
Catalogue discovery uses A74's source list/selection/search projections and
CLI pagination, filtering and candidate-cache publication regressions.
...
Catalogue HTTP failures use A78's actual source consumers, redirect error-message
comparisons, wire reason/body regressions and CLI failure/publication cases.
Discovery JSON uses A79's raw-document source comparisons and CLI decoding,
pagination, candidate publication and metadata-probe regressions.
Catalogue model JSON uses A80's raw model/cache-text and GraphQL-envelope
comparisons, surrogate path/OS probes and CLI publication/acquisition regressions.
...
### GitHub catalogue model validation and normalized metadata

Under A73, `github/models.rs` defines the complete repository, release, tag, commit
and asset records. `github/models/graphql.rs` converts GraphQL fields before
applying those validators. Cached records use the same complete models and
upstream `model_dump()` field names. The primary contracts are
`GitHubReleaseAsset`, `GitHubRelease.from_dict`, `GitHubTag.from_dict`,
`GitHubCommit.from_dict`, `GitHubReleases` and their construction in
`GitHubGraphQLClient.get_many_releases`, all in the pinned
`src/hcli/lib/ida/plugin/repo/github.py`.

Release conversion now validates created/published strings, flags, URL, name,
tag name, source URL/hash and assets, including fields unused by archive filtering.
Absent or falsey release names fall back to the tag name; absent tag names default
to the empty string. Missing asset connections or missing asset nodes default to
an empty list. GraphQL collection comprehensions accept empty objects/strings;
cached Pydantic list fields require arrays for represented JSON input.
Required null fields fail instead of silently suppressing a release.

Release and tag targets unwrap exactly one `target` field, which takes precedence
over any outer commit fields. Default-branch conversion does not unwrap tags.
Release sources require a hash and ZIP URL but do not require the target's commit
date; default commits and tags do. The old recursive target enum is removed.
Acquisition consumes these validated records directly, preserving A70's ordering,
multiplicity and archive identities.

Asset aliases take precedence over snake-case field names, including when the
alias contains an invalid value. Boolean fields use the existing Pydantic-compatible
converter. The existing integer type moved from API-specific code to
`util/pydantic_integer.rs`; API asset types retain their public re-export, and
catalogue assets now use the same parser and signed arbitrary-precision storage.
The 104,857,600-byte size comparison operates on that integer. Negative sizes do
not exceed the source limit; large positive sizes remain valid metadata and are
skipped on a cold asset-cache lookup. Cached bytes still precede this size check.

Metadata resource keys advance from `releases-v2` to `releases-v3`. Earlier entries
omit required fields, so they cannot establish a complete source model and are
not read as the new schema. They remain on disk and require an online metadata
refresh; offline-only operation cannot reconstruct the missing values. Archive
cache identities and bytes are retained. The new model values match represented
upstream records, but hashed account/origin paths and compact native JSON remain
different from upstream's physical paths and indented/sorted cache text.

Evidence:

- 910 read-only comparisons invoke the actual upstream `from_dict` methods or
  `GitHubReleases.model_validate` and compare success/failure and complete
  `model_dump()` values. They cover missing/null/wrong-type fields, falsey names,
  optional asset connections, empty collections, boolean spellings, integer
  coercion and large/signed sizes, alias collisions and target depth/precedence.
  These compare model outcomes and values, not exception classes/details or
  complete raw JSON decoder acceptance.
- The moved integer test still matches 37,498 inputs against Pydantic 2.12.5 in
  the locked runtime. Catalogue model comparisons separately use the A45 runtime's
  Pydantic 2.13.5; this does not establish exhaustive equality between versions.
- Six CLI scenarios remove required release fields and verify failure before
  metadata publication or archive acquisition.
- A CLI round-trip accepts source boolean/name/integer coercions, prefers aliases,
  downloads a negative-size asset, skips a huge positive-size asset, persists
  normalized metadata and performs no requests on the second invocation.
- A CLI migration fixture retains an incomplete v2 entry while fetching one
  replacement metadata response and publishing a complete v3 record.
- Existing 550 acquisition-plan, 113 GraphQL-envelope, 562 retry and 33 cache-read
  source comparisons remain applicable; catalogue fixtures now supply complete
  valid release fields rather than relying on the old partial native model.

Bounded findings: **high impact** — unused required fields still decide whether
the source accepts the repository, so a partial native schema can acquire archives
that upstream rejects. **Medium impact** — recursively unwrapping tags or selecting
outer commit fields changes source URL/hash identity. **Medium impact** — unsigned
sizes reject valid upstream models; the size limit is a later acquisition check.
**Medium impact** — complete metadata needs a new online refresh after migration.
**Low impact** — retained legacy files continue consuming disk space.

For N records and B represented bytes, conversion traverses O(N + B) fields and
uses O(N + B) model/intermediate storage, excluding integer-conversion costs.
D-digit decimal integer parsing has an O(D²) upper bound and O(D) storage; the
existing 4,300-digit conversion rules are unchanged. JSON parser limits,
surrogates/nonfinite constants, exhaustive coercion/error precedence, source cache
text/path/write behavior and native Windows execution remain open under QG3/QG5.

The uninterrupted A73 serial all-target run passed 724 tests: 277 unit tests and
447 integration tests across 62 suites, with nine existing opt-in tests ignored
and no failures. The prior four read-only source-oracle environments remained
enabled, with `HY_TEST_ASSET_ORACLE_PYTHON` additionally set to the locked Pydantic
runtime for the moved integer parser and its existing consumers. Formatting,
whitespace checks, Clippy with warnings denied and Windows all-target
cross-compilation passed. Logs are `/tmp/hy-github-model-source.log`,
`/tmp/hy-github-model-integers.log`, `/tmp/hy-github-model-cli.log`,
`/tmp/hy-github-model-full.log`, `/tmp/hy-github-model-clippy.log` and
`/tmp/hy-github-model-windows.log`. The source checkout remained clean at the
pinned revision. Full project parity remains open.

### GitHub discovery, repository lists and validation order

Under A74, `github/discovery.rs` owns search pagination, list parsing, selection
and repository-name checks. The primary contracts are `read_repos_file` in
`src/hcli/commands/plugin/__init__.py`, together with
`find_github_repos_with_plugins`, `GithubPluginRepo._get_repos`, `parse_repository`
and metadata cache lookup in the pinned `src/hcli/lib/ida/plugin/repo/github.py`.
Cache-component grammar comes from `src/hcli/lib/util/cache.py::validate_path_component`.

Search sends the two source queries in order, with percent-encoded colons/spaces,
`per_page=100`, one-based page numbers, the GitHub v3 Accept header and the source
`ida-hcli` search User-Agent. Authentication remains on metadata requests.
Missing or falsey `items` ends that query. Nonempty represented pages collect
repository strings and continue when the item count is at least 100; the count is
taken before repository deduplication. Names are lowercased and cached after both
searches, before selected repository names are parsed. Cached names are lowercased
again when loaded, matching the source's warm-cache branch.

List files retain UTF-8 BOM characters and use Python's line boundaries and trim
characters. Empty lines and lines starting with `#` are filtered before trimming.
Consequently, whitespace-only lines contribute an empty name, and indented comments
contribute names beginning with `#`. File loading itself performs no repository-name
validation. Selection lowercases candidate/extra/ignored sets, adds extras, removes
ignored names and only then requires exactly one `/` in each retained name.
Invalid ignored names that never survive selection do not abort the command.

Owner/repository cache-component checks occur during metadata warming. The source
rejects empty/dot components, non-ASCII characters, tabs/newlines and slashes;
other ASCII punctuation and spaces are not rejected by this lexical rule. Native
cache storage still uses hashed paths, so passing these lexical checks does not
certify source filesystem-name behavior. Query construction now interpolates
owner/name text as the source does, rather than introducing JSON string escaping
inside GraphQL. This matches construction even for represented names that a real
GraphQL server would reject; it does not establish their live GitHub validity.

Evidence:

- 165 read-only source comparisons comprise 77 list cases, 54 selection cases,
  15 cache-component cases and 19 search response/pagination cases. The actual
  source list reader uses intercepted `Path.exists/read_text`; selection invokes
  `_get_repos` with an intercepted candidate getter. Search invokes the actual
  discovery function with intercepted HTTP responses and records request URLs
  and source headers. No Python filesystem writes are permitted.
- The existing query/envelope comparison expands from 113 to 117 cases, adding
  spaces, quotes, backslashes and a hash character in repository names.
- A CLI fixture verifies exact search paths and headers, continuation after 100
  duplicate hits, termination on missing/null items, deduplication and zero requests
  on a subsequent cache hit.
- A CLI fixture combines Unicode/CRLF line splitting, whitespace-only lines,
  indented comments, invalid discovered/extra/ignored names and a space-bearing
  repository name. Only the two retained repositories reach GraphQL.
- Two CLI failure/recovery scenarios show that discovery is cached before malformed
  repository or cache-component failures. Adding a matching ignore entry then
  succeeds without repeating the search requests. The existing invalid-list test
  now uses an owned HTTP endpoint and checks this source ordering.

Bounded findings: **high impact** — validating names before applying ignored entries
rejects configurations that upstream accepts. **Medium impact** — trimming before
comment/empty-line filtering silently changes the contents of repository lists.
**Medium impact** — pagination based on unique names misses later pages when the
first page contains duplicate hits. **Medium impact** — discovery cache publication
survives later selection failures. **Low impact** — search uses its explicit source
User-Agent; GraphQL/archive transport identity remains a separate contract.

For B list/response bytes and N names with comparison length L, parsing costs O(B),
set construction/selection costs O(N log N × L), and retained names take O(B) storage
in addition to response/model buffers. Each query requests pages until it observes
a short/falsey page or an error. No additional page quota or deadline is imposed;
the native page counter is unsigned 64-bit. Non-string search names still have
different failure timing in some cases: upstream can collect hashable scalars and
fail during later sorting/lowercasing, while native validation rejects them within
the page. Non-array candidate-cache roots, complete JSON lexical behavior, Unicode
version differences, physical cache paths and diagnostic text remain open under
QG3/QG5.

The uninterrupted A74 serial all-target run passed 729 tests: 279 unit tests and
450 integration tests across 62 suites, with nine existing opt-in tests ignored
and no failures. All five A73 source-oracle environment variables were retained.
Formatting, whitespace checks, Clippy with warnings denied and Windows all-target
cross-compilation passed. Logs are `/tmp/hy-github-discovery-source.log`,
`/tmp/hy-github-discovery-cli.log`, `/tmp/hy-github-discovery-full.log`,
`/tmp/hy-github-discovery-clippy.log` and `/tmp/hy-github-discovery-windows.log`.
The source log records the initial 113-case GraphQL corpus; the full run includes
the final 117-case expansion. The upstream checkout remained clean at its pinned
revision. Full project parity remains open.

### GitHub catalogue cache interoperability and publication

Under A75, catalogue caches use the upstream physical layout beneath the configured
HCLI cache root. The primary contracts are `get_cache_directory` and
`validate_path_component` in `src/hcli/lib/util/cache.py`, plus all candidate,
release-metadata, release-asset and source-archive cache getters/setters in the
pinned `src/hcli/lib/ida/plugin/repo/github.py`.

| Resource | Relative cache path |
| --- | --- |
| Candidate repository list | `candidate_repos.json` |
| Complete repository metadata | `<owner>/<repo>/releases.json` |
| Release asset | `<owner>/<repo>/release-assets/<release-tag>/<asset-name>` |
| Source archive | `<owner>/<repo>/source-archives/<commit>/source.zip` |

Token/API-origin hashing is removed. Different tokens and native API-origin
overrides now consult the same files under a shared root. Existing upstream files
are read directly; no conversion or network request is required for represented
fresh, complete entries. Legacy `github-catalogue` hashes are retained but are not
consulted. If no upstream-layout entry exists, acquisition must populate it;
offline-only operation cannot recover data solely from those legacy hashes.
This supersedes the account/origin partition and versioned native metadata-key
descriptions under A70–A74.

Cache-directory lookup validates components and creates directories before checking
for files, including cold read misses. Owner, repository, release-tag and commit
components follow the source ASCII/dot/separator rules. Invalid archive cache keys
raise the represented ValueError and are skipped by the catalogue caller before
downloading that archive. Metadata-key failures remain fatal. Asset filenames
are joined with the shared pathlib spelling helper; they are not cache-directory
components. Their extra parent directories are not created by publication.

Publication writes directly to the destination, following existing file symlinks;
atomic temporary-file replacement is removed for these caches. The source cache
directory is looked up again before a fresh archive is published. Filesystem
errors propagate, while represented ValueError failures anywhere in an archive
getter are skipped. A75 retains A72's missing/read/error, expiry and clock rules.
These changes do not certify every partial-write or concurrent replacement state.

Candidate and metadata JSON now use recursively sorted keys, two-space indentation,
ASCII escapes, source integer spelling and no appended newline. Native Windows
text publication translates layout newlines to CRLF. The formatter moved to
`util/json_format.rs`; snapshot output still uses four spaces and retains its
existing integer-limit check. Catalogue models contain integer sizes and no float
fields, so the snapshot formatter's Pydantic nonfinite-to-null policy is not
exercised by valid catalogue records.

Evidence:

- 47 read-only source comparisons cover 36 directory-component cases, seven
  filename joins and four complete candidate/metadata JSON texts. Actual source
  setters generate the text with `Path.write_text` intercepted; mkdir is also
  intercepted. The path corpus includes invalid components, spaces/punctuation,
  nested/parent/dot/absolute asset filenames and Unicode filenames. These are
  path/text projections, not execution of source writes or arbitrary external paths.
- A CLI writes candidates, complete metadata, release bytes and source bytes to
  the source layout. Actual upstream getters then read those files and validate
  the model and both archive SHA-256 values. Only mkdir calls for already existing
  directories are intercepted. A second native invocation changes both token and
  API origin and performs no network requests.
- A CLI fixture preloads upstream-format candidate/metadata files and a source ZIP;
  Hy constructs the expected plugin catalogue without contacting its HTTP endpoint.
- Four invalid release-tag scenarios skip the asset before its HTTP request while
  acquiring the valid source archive.
- A nested asset filename downloads successfully but fails publication without
  creating the missing filename parent; the release cache directory remains.
- A Unix dangling-file-symlink fixture verifies that candidate publication retains
  the symlink and creates its target. Existing expiry, cache-corruption, metadata
  batching and acquisition tests were updated to use the shared layout.
- Snapshot's 1,112,064-Unicode-scalar and 90-layout/number comparisons continue to
  exercise the shared formatting implementation.

Bounded findings: **high impact** — tokens sharing a cache root now reuse the same
discovery and archive entries, matching upstream. **High impact** — release/commit
cache-key validation can suppress a download even when its URL and metadata are
otherwise valid. **Medium impact** — direct writes preserve file-symlink behavior
and can expose partial output on failure. **Medium impact** — a missing nested
asset parent remains a publication error after downloading. **Medium impact** —
legacy-only native caches require refresh into the shared layout. **Low impact** —
legacy files remain on disk until separately removed.

Path construction scans O(P) component/filename bytes. Reading/writing B payload
bytes takes O(B) work and retained buffers; JSON output retains O(B) text/model
storage plus key sorting, with integer-conversion costs described under A73.
Directory and file operations add filesystem-dependent costs. No lock, quota or
new deadline is introduced. JSON lexical/model limits, concurrent and partial I/O,
unusual OS aliases, native Windows behavior, urllib transport and remaining
discovery/diagnostic differences keep QG3 and QG5 open.

The uninterrupted A75 serial all-target run passed 735 tests: 280 unit tests and
455 integration tests across 62 suites, with nine existing opt-in tests ignored
and no failures. All five A73 source-oracle environment variables were retained.
Formatting, whitespace checks, Clippy with warnings denied and Windows all-target
cross-compilation passed. Logs are `/tmp/hy-github-cache-layout-source-final.log`,
`/tmp/hy-github-cache-layout-cli-final.log`, `/tmp/hy-github-cache-layout-full.log`,
`/tmp/hy-github-cache-layout-clippy.log` and
...
### GitHub catalogue HTTP failure bodies and reason phrases

Under A78, JSON callers identify their endpoint explicitly (A79 replaces the
`JsonEndpoint` enum with separate typed search and GraphQL reader functions).
GraphQL catches HTTP failures outside the retry loop, reads the final body,
decodes it strictly as UTF-8 and reports `HTTP <status>: <body>`. Bodies retain
their original newlines, BOM and other valid text; Content-Type charset labels
do not change decoding. A read failure or UnicodeDecodeError replaces the HTTP
status error and does not restart acquisition. Search and release/source archive
failures instead report `HTTP Error <status>: <reason>` without reading the body.
The primary contracts are the four acquisition functions and GraphQL client's
`query` in the pinned upstream catalogue module.

HTTP/1 reason phrases use CPython's Latin-1 decoding and Unicode whitespace
stripping. Redirect rejection retains urllib's forbidden-scheme explanation or
its multiline loop explanation. GraphQL still reports the response body for
these HTTPError instances; it does not substitute the redirect explanation.
`util/python_utf8.rs` extracts the existing strict decoder from subprocess text
handling. Subprocess callers retain their own universal-newline conversion;
HTTP error bodies do not acquire that conversion. A43's 86,279 text comparisons
and 242 version outcomes remain the existing regression evidence for that path.

The wire regression identified a dependency-level loss of data: httparse 1.10.1
validates HTTP obs-text (0x80–0xFF) in a reason phrase but returns an empty string
for the entire phrase. Hyper 1.10.1 then stored that empty string in its response
extension. Application-level decoding alone could not recover the bytes.

Hyper 1.10.1 is therefore retained locally under `vendor/hyper`, with one
implementation file changed. After httparse validates a complete response, the
patch recovers a raw reason only when the parser returned an empty string and
the validated status line contains obs-text in its reason. Existing ASCII and
empty reason handling, status/header validation and message framing
use the previous paths. Both the helper and client implementation keep their
client feature gates. No new unsafe code is introduced.

Provenance and review:

- The published crate checksum was verified as
  `55281c53a1894c864990125767da440a4e630446785086f52523b20033b74498`;
  its recorded upstream revision is `e3bcd379118e707b3e49185b047d49ebbaa943fc`.
- Of 68 retained upstream files, only `src/proto/h1/role.rs` differs. This was
  checked byte-for-byte against the installed registry copy, including the
  manifests, README and MIT license. The crate-local rustfmt configuration keeps
  upstream formatting separate from Hy's settings.
- `vendor/hyper/PATCHES.md` documents the change and removal condition.
  `vendor/hyper/reason-phrase.patch` records the exact zero-context delta; its
  reverse application check passes against the working tree.
- Cargo.lock changes only Hyper's registry-source/checksum fields. Unrelated
  Windows dependency rewrites produced during resolution were removed, and the
  resulting pinned lockfile passes offline locked builds.

Evidence:

- 888 actual-source comparisons cover ten error statuses and all four consumers,
  ten body values, simulated incomplete reads, and 448 reason-byte projections.
  They compare complete HTTP/UTF-8 error messages and whether the body was read.
  Read-exception classes are compared by their failure boundary; native network
  error text is not equated with Python's IncompleteRead text. Source transport
  is intercepted and the oracle rejects filesystem mutation/network access.
- A77's 927 source cases now also compare redirect rejection messages, including
  ordinary method rejection, forbidden schemes, and repeated/distinct-target
  loop errors. Their previous request/header/history assertions remain intact.
- Eight real socket cases combine canonical, Latin-1, empty and Unicode-whitespace
  reasons with search/GraphQL error policies and truncated bodies. This regression
  failed before the dependency patch and passes with raw reason preservation.
- Four CLI tests cover sixteen scenarios: six unmodified GraphQL error texts,
  three strict decoding failures, three search/archive failures with invalid UTF-8
  bodies, and four forbidden-scheme redirects. They assert request counts and
  candidate/metadata/archive publication boundaries. The previous CLI loop tests
  now assert the full source explanation.

Bounded findings: **high impact** — GraphQL service explanations are observable
instead of a generic status. **Medium impact** — malformed GraphQL error bodies
can replace the status failure, while the same archive/search bodies remain
unread. **Medium impact** — a transport dependency can discard reason bytes before
application code sees them. **Medium impact** — the local Hyper patch requires
comparison when that dependency is upgraded and affects shared HTTP consumers.

Reading a B-byte GraphQL error body and formatting its message takes O(B) time
and storage. Reason recovery scans the validated status line in O(L) time for
line length L; the application then decodes/trims its retained bytes in O(L).
No extra retry, body quota, deadline or network request is introduced. Arbitrary
malformed HTTP framing, additional URL/transport exception mappings and text,
FTP, native URL normalization and other-platform runtime behavior keep QG3/QG5
open.

The uninterrupted A78 serial all-target run, with the patched Hyper dependency,
passed 753 tests: 286 unit tests and 467 integration tests across 62 suites, with
nine existing opt-in tests ignored and no failures. All five A73 source-oracle
environment variables were retained. The 888-case comparison and wire regression
were rechecked after Clippy's equivalent test-iterator cleanup. Formatting,
whitespace checks, Clippy with warnings denied and Windows all-target
cross-compilation passed. Logs are `/tmp/hy-github-http-failure-source-patched.log`,
`/tmp/hy-github-http-failure-cli.log`, `/tmp/hy-github-http-failure-full.log`,
`/tmp/hy-github-http-failure-final.log`, `/tmp/hy-github-http-failure-clippy-final.log`
and `/tmp/hy-github-http-failure-windows-final.log`. The earlier targeted CLI log
predates the dependency patch; the full run includes all CLI cases with it. The
upstream checkout remained clean at its pinned revision. Full project parity
remains open.

### GitHub discovery JSON values and text decoding

Under A79, search responses and candidate-cache contents use the existing
`util/python_json` representation. They decode strict UTF-8 before parsing JSON
text, as the pinned source does. This differs from direct-release byte decoding
under A67: a UTF-8 BOM or UTF-16/32 body is not accepted at this text boundary.
The primary contracts are `find_github_repos_with_plugins`,
`get_candidate_github_repos_cache`, `set_candidate_github_repos_cache`,
`GithubPluginRepo._get_repos` and `warm_releases_metadata_cache` in
`src/hcli/lib/ida/plugin/repo/github.py`, plus the cache component validator in
`src/hcli/lib/util/cache.py`, at the pinned comparison revision.

Ignored fields may contain NaN, positive/negative Infinity, overflowing binary64
numbers or unpaired surrogate strings. Selected scalar numbers still cause a
failure after both search queries and all pages finish; arrays/objects remain
immediate failures. Strings retain Unicode code points through deduplication,
ordering, lowercasing, filtering and candidate-cache publication. Scalar runs are
lowercased together to preserve contextual final sigma; surrogate code points
separate those contexts. Candidate serialization shares ASCII escaping with the
existing snapshot/metadata formatter and represents unpaired surrogates directly
as JSON escapes. It preserves two-space indentation, no trailing newline and the
platform text-newline convention.

Repository parsing still precedes all metadata probes. Component validation
occurs during each ordered cache lookup, so an invalid surrogate-containing name
does not suppress earlier directory creation or cache-read failures. Only names
that have passed this boundary become Rust strings for GraphQL and acquisition.
Search and GraphQL now have separate response readers; the existing GraphQL
success/model decoder is unchanged and remains a separate parity gap.

Evidence:

- 276 actual-source comparisons use raw response/cache bytes and compare selected
  name code points, request URLs and exact publication text. They include all
  2,048 unpaired surrogate code points, contextual Greek casing, malformed JSON,
  duplicate keys, nonfinite constants, overflow, 4,300/4,301-digit integers,
  128/512 nested containers, BOMs, UTF-16/32 text and invalid UTF-8. The source
  oracle intercepts HTTP and file reads/writes; its audit hook rejects actual
  filesystem mutation. No Python process edits files.
- The prior 165 discovery and 112 container/timing comparisons pass with the new
  representation. Their ordinary scalar fixtures still compare complete values.
- Four CLI tests cover 24 invocations: ignored Python values and cache reuse,
  deferred nonfinite-name failure over both paginated queries, surrogate cache
  publication before parsing/component errors, ordered directory probes, and
  strict text decoding without refresh or cache replacement after failure.
- The 46-test catalogue integration suite, warnings-as-errors Clippy and Windows
  all-target cross-compilation pass. Native Windows execution remains unverified.

Bounded findings: **high impact** — Python-only values in unused fields previously
rejected otherwise usable discovery results. **Medium impact** — converting strings
to UTF-8 before publication changed which cache artifacts and earlier repository
directories survived a failure. **Medium impact** — byte-encoding autodetection
would incorrectly broaden the source's explicit UTF-8 text boundary. **Low impact**
— shared ASCII escaping avoids a separate candidate-cache string formatter.

For B document code points, N name entries, K retained names of maximum length W
and R output bytes, decoding/container traversal takes O(B) work/storage, excluding
bounded integer-conversion costs. Ordered name operations have an
O(N W log(K + 1)) comparison bound, including duplicate entries; casing and
publication scan retained/output text. Storage is O(B + K W + R).
The existing arbitrary-precision parser retains the 4,300-digit default bound;
its integer conversion has an O(D²) upper bound for D decimal digits. No document
quota or CPython runtime-dependent recursion limit is introduced. Alternate
Unicode/runtime versions, cache-text locales, resource failures, exact exception
diagnostics, GraphQL/model JSON and native platform execution keep QG3/QG5 open.

The uninterrupted serial all-target run passed 758 tests: 287 unit tests and 471
integration tests across 62 suites, with nine existing opt-in tests ignored and
no failures. All five A73 source-oracle environment variables were enabled. This
includes the shared formatter's all-Unicode scalar comparison, the existing
2,012-case byte decoder and the catalogue HTTP/redirect/cache/model regressions.
The new 276-case oracle passed again after an equivalent test readability cleanup;
final formatting, whitespace, warnings-as-errors Clippy and Windows all-target
checks also passed. Logs are `/tmp/hy-discovery-json-source.log`,
`/tmp/hy-discovery-json-cli.log`, `/tmp/hy-discovery-json-full.log`,
`/tmp/hy-discovery-json-final.log`, `/tmp/hy-discovery-json-clippy-final.log` and
`/tmp/hy-discovery-json-windows-final.log`. The upstream checkout remained clean
at the pinned revision. Full project parity remains open.

### GitHub GraphQL and metadata-cache Python values

Under A80, successful GraphQL bodies and metadata-cache text use the same strict
UTF-8/Python JSON representation as discovery. The primary contracts are
`GitHubGraphQLClient.query`, `get_many_releases`, the five GitHub Pydantic models,
the metadata-cache getters/setters and archive acquisition functions in pinned
`src/hcli/lib/ida/plugin/repo/github.py`. CPython 3.13.15 supplies JSON, repr,
filesystem encoding and pathlib behavior; the source models use the runtime
recorded under A45.

Model declarations retain Python `Text` strings. `models/fields.rs` owns common
field coercion; `models/cached.rs` validates canonical cached records, while
`models/graphql.rs` extracts GraphQL fields and preserves iterable/fallback
rules. Ignored fields can contain nonfinite numbers, large integers, deep
containers or unpaired surrogates. Required numeric/boolean/string fields retain
their Pydantic acceptance rules, and aliases keep presence-based precedence.
NaN remains truthy rather than becoming null and taking a fallback branch.

`models/serialization.rs` builds the canonical model-dump fields and the shared
JSON formatter emits sorted, two-space, ASCII-escaped text. Unpaired surrogates
remain JSON escapes through publication and later cache reuse. Serialization
does not depend on declaration order or a UTF-8-only serde value intermediate.
No global serde feature or lockfile change is introduced. Test-only serde
projections retain the prior ordinary-scalar model/acquisition comparisons.

GraphQL fatal-error rendering uses an iterative Python-value repr writer, retaining
object insertion order, quoting, surrogate escapes, arbitrary integers and
`nan`/`inf` spelling. Complete batch validation still precedes publication.
Release/tag dates compare code points, MIME-type selection uses exact strings,
and ZIP suffix selection operates without converting whole names to UTF-8.
ASCII cache-key validation therefore rejects surrogate tags/commits at the
archive getter, after metadata publication, without suppressing other archives.

Asset filenames have a separate filesystem boundary. POSIX joining preserves
pathlib components and applies UTF-8 with surrogateescape for U+DC80–U+DCFF.
Other unpaired surrogates produce a false existence probe, so acquisition can
download before publication raises the source's caught ValueError equivalent.
An OS error remains fatal. On the exercised macOS filesystem, U+DCFF becomes
byte FF, its missing-name probe succeeds as a miss, and creation then raises
EILSEQ after download. The CLI fixture probes the owned filesystem to distinguish
that behavior from filesystems that permit raw-byte filenames. Windows encodes
surrogates as native UTF-16; its unusual-name normalization/runtime remains open.

Evidence:

- 882 raw-model comparisons mutate every field/container of GraphQL and cached
  fixtures with nonfinite values, overflow, signed zero, surrogate strings and
  malformed types. They also cover ignored fields, 512 nested containers,
  4,300/4,301-digit integers and every unpaired surrogate code point. Successful
  models compare complete publication text with `json.dumps(model_dump(),
  indent=2, sort_keys=True)`; failures compare acceptance, not aggregate reports.
- 116 raw GraphQL-response comparisons call the actual upstream client and compare
  model cache text or full fatal-error text. They cover Python truthiness, error
  filtering, nonfinite/surrogate repr and rejection of BOM/non-UTF-8 text.
- 10,240 POSIX surrogate-path comparisons cover five path patterns for each of
  2,048 surrogate code points. Two read-only native existence probes distinguish
  unencodable strings from OS-encodable raw-byte names. Rust owns all filesystem
  fixture creation/removal; Python oracles reject mutation.
- The existing 910 model, 117 GraphQL and 550 acquisition-plan comparisons pass
  with the typed representation. Five CLI tests cover nine scenarios and thirteen
  macOS invocations: cache reuse, ignored nonfinite fields, old-release filtering,
  invalid archive keys, filename error/publication order, whole-batch rejection
  and fatal-error rendering.

Bounded findings: **high impact** — ignored Python-only JSON values no longer reject
an otherwise valid metadata batch. **High impact** — model strings survive cache
publication and filtering before later path validation. **Medium impact** — an
unencodable Python string and an OS-rejected filename have different acquisition
boundaries and exception policies. **Medium impact** — serde's optional raw-value
feature recognizes specially named object keys; catalogue serialization avoids
changing decoding semantics of unrelated consumers.

For B decoded document/model code points and R rendered bytes, traversal and
rendering take O(B + R) work/storage, excluding integer arithmetic. Model objects
have a fixed field count; key sorting is bounded per object. Integer conversion
retains the O(D²) upper bound and 4,300-digit default under A12. Error repr uses
an explicit stack; model-cache serialization has schema-bounded nesting. Native
filename encoding/normalization is O(L) work/storage for L code points. No new
network deadline, byte quota or runtime-dependent Python recursion limit is added.

At A80, an already cached archive whose URL contained unpaired surrogates still
reached a UTF-8-only archive-index interface; A81 below addresses that boundary.
Surrogate file URLs still need transport/path handling. Native Windows unusual-name joining,
other filesystem/runtime/locale profiles, complete JSON resource boundaries and
exact aggregate/outer exception diagnostics remain unverified or incomplete.
QG3 and QG5 therefore remain open.

The uninterrupted serial all-target run passed 766 tests: 290 unit tests and 476
integration tests across 62 suites, with nine existing opt-in tests ignored and
no failures. All five A73 source-oracle environments were enabled. The 51-test
catalogue integration suite passes in this full run. Final formatting, whitespace,
warnings-as-errors Clippy and Windows all-target cross-compilation also pass.
Logs are `/tmp/hy-catalogue-json-source-final.log`,
`/tmp/hy-catalogue-json-paths-final.log`, `/tmp/hy-catalogue-json-cli-final.log`,
`/tmp/hy-catalogue-json-full.log`, `/tmp/hy-catalogue-json-clippy-final.log` and
`/tmp/hy-catalogue-json-windows-final.log`. Earlier CLI logs contain the initial
filesystem-assumption failures described above; the final fixture distinguishes
probe and creation errors. The upstream checkout remained clean at the pinned
revision. Full project parity remains open.

### Archive-index URL identity and consumer serialization

Under A81, `Location.url` retains Python code points through archive indexing,
filtering, sorting and selection. The primary contracts are `PluginArchiveIndex`
and `PluginArchiveLocation` in pinned `src/hcli/lib/ida/plugin/repo/__init__.py`,
`JSONFilePluginRepo.to_json` in `repo/file.py`, the exact-version result model and
`_dump_result` in `commands/plugin/search.py`, and `print_json` in
`lib/console.py`. Source comparisons use the A45 runtime.

The index no longer encodes a cached archive URL before reading its descriptors.
Archives rejected by host filtering require no URL encoding. Later archives are
acquired and indexed before a consumer serializes the completed catalogue.
Sorting compares URL code points and then checksum, preserving upstream's
metadata-comparison failure when both preceding tuple fields tie.

Snapshot export and search have distinct contracts. Upstream snapshot export
passes through Pydantic `model_dump_json`, which rejects unpaired surrogates;
native snapshot serde serialization retains that rejection. Pydantic JSON model
input also rejects escaped unpaired surrogates, matching native snapshot input.
Exact-version search instead uses `model_dump(mode="json")` followed by Python
`json.dumps`: its URLs remain escaped strings in successful JSON output. A
dedicated search serialization module uses the shared Python-value formatter
with insertion order and ASCII escaping. No global serde feature changes.

Plain Unix download-location output uses UTF-8 with surrogateescape: U+DC80–U+DCFF
map to bytes 80–FF, while other unpaired surrogates fail at output. The shared
codec also replaces the existing duplicate filesystem encoder. Non-Unix output
retains strict UTF-8 conversion; native Windows console behavior and alternate
Python stdout settings are not certified by this implementation. Selected URLs
still cross a strict UTF-8 boundary at archive fetch, so surrogate file-URL
transport remains a separate gap.

Evidence:

- 2,053 index cases cover every one of the 2,048 surrogate code points plus empty,
  ASCII, escaped and non-ASCII scalar strings. The actual source index compares
  ordered URL code points, selected display name, complete successful snapshot
  text or serialization failure, and JSON model-input acceptance.
- 2,050 exact-version report cases compare full output from the source result
  model, `_dump_result` and `print_json`, including field order and escaping.
  They also compare UTF-8/surrogateescape bytes or encoding failure.
- Three CLI tests cover seven scenarios and twenty-three macOS invocations.
  Cached asset and source URLs survive later acquisition; snapshot serialization
  fails only after the later archive is cached. Keyword/version searches and
  exact-version JSON succeed. Plain Unix output distinguishes high surrogates
  from surrogateescape bytes. A later HTTP 401 precedes snapshot serialization
  failure, and a host-filtered archive produces an empty successful snapshot.
  All HTTP servers and cache files are owned test fixtures; Python oracles use
  isolated execution, disabled bytecode writes and mutation-rejecting audit hooks.

Bounded findings: **high impact** — premature URL encoding could prevent later
downloads and reject archives that filtering would discard. **Medium impact** —
URL code-point identity controls location ordering and tie handling. **Medium
impact** — applying one JSON serializer to both commands would either reject
valid search output or incorrectly permit snapshot export.

For B retained code points and R output bytes, encoding and report traversal take
O(B + R) work and storage. URL storage uses four bytes per code point, excluding
container overhead. Sorting N locations takes O(N log N) comparisons under the
existing index algorithm, with O(L) worst-case URL comparison for maximum URL
length L. Report serialization has schema-bounded model nesting; no new response
quota, Python runtime limit or network deadline is introduced.

The uninterrupted serial all-target run passed 771 tests: 292 unit tests and 479
integration tests across 62 suites, with nine existing opt-in tests ignored and
no failures. All five A73 source-oracle environments were enabled. The final
JSON-input extension and test readability cleanup were checked with the targeted
source/CLI suites. Formatting, whitespace, warnings-as-errors Clippy and Windows
all-target cross-compilation pass. Logs are `/tmp/hy-index-urls-full.log`,
`/tmp/hy-index-urls-source-final.log`, `/tmp/hy-index-urls-report.log`,
`/tmp/hy-index-urls-cli.log`, `/tmp/hy-index-urls-final.log`,
`/tmp/hy-index-urls-clippy-final.log` and `/tmp/hy-index-urls-windows-final.log`.
The upstream checkout remained clean at the pinned revision. Alternate codecs,
native platform execution, surrogate file-URL transport and exact outer error
diagnostics remain open; this group does not close QG3 or QG5 for full parity.

### Catalogue urllib file requests and acquisition boundaries

Under A82, catalogue file downloads have their own request, path and host modules
in `src/plugin/index/github/file`. The primary HCLI contracts are
`download_release_asset`, `download_source_archive`, their cache wrappers and
`_urlopen_with_retry` in pinned `src/hcli/lib/ida/plugin/repo/github.py`. Those
functions use `urllib.request.Request` and `FileHandler`; they do not use
`fetch_plugin_archive` and its pathlib conversion. The lexical adapters follow
CPython 3.13.15 `Lib/urllib/request.py`, `Lib/urllib/parse.py` and
`Lib/nturl2path.py`, with adaptation provenance in `src/util/python_path/LICENSE`.

Request parsing unwraps surrounding whitespace, angle brackets and the `URL:`
prefix, recognizes case-insensitive `file`, removes the fragment, decodes the
host and retains query/control text in the selector. Its origin-host validation
uses the existing urllib authority checks. Python code points remain intact;
surrogates in a discarded fragment do not prevent a successful file download.

Unlike ordinary archive fetches, catalogue paths retain empty, dot and trailing
components through OS lookup. POSIX conversion percent-decodes filesystem bytes
with UTF-8/surrogateescape. Windows conversion retains literal surrogate code
points, applies nturl2path drive rules, and replaces malformed percent-encoded
UTF-8 in the same way as `unquote`. No pathlib normalization is applied. Embedded
NUL and Unix-unencodable surrogate paths produce the source ValueError category,
which acquisition skips; an OS stat/open failure produces the retryable URLError
category and eventually stops acquisition.

The file handler checks `stat` before ordinary host resolution. A nonempty
numeric port prevents local acceptance. Hosts without a port resolve to IPv4 and
must match the cached localhost/machine address set. The double-leading-slash
selector branch has a separate, literal address check: a recognized local
address still falls through to urllib's unknown-handler error. Native hostname
calls use bounded initialized buffers; successful Winsock initialization is
balanced by cleanup. The Windows dependency enables only the additional Winsock
API feature; dependency versions and Cargo.lock are unchanged.

Stat/open URLError failures receive four total attempts with waits of 2, 4 and
8 seconds, totaling 14 seconds of configured delay. ValueError and other terminal
failures do not retry. A returned file is read outside that boundary, matching
the source response consumption. On Unix the opened descriptor is checked for a
directory before returning, so directory-open failure remains inside retries.
Cache publication follows a successful complete read. Cache hits and download
size checks retain their earlier acquisition order.

Evidence:

- 12,040 source comparisons cover wrapper/scheme variants, authorities, ports,
  queries/fragments, separators, drive spellings, percent escapes, control text
  and all 2,048 surrogate code points in five positions. They compare parsed
  host/selector code points, POSIX bytes, Windows code points and the special
  double-slash branch decision, including parse/conversion failures.
- 72 source acquisitions execute the actual HCLI download function and urllib
  handler against Rust-owned files. They compare payloads, exception categories,
  OS-versus-host failure phases, request attempt counts and the exact wait
  schedule. Cases include trailing separators, directories, missing files,
  lexical symlink parents, literal queries/tabs, percent-encoded NUL/invalid bytes,
  local/nonlocal numeric hosts, ports and surrogate paths/hosts. Python sleep is
  observed without wall-clock delay; mutation-rejecting audit hooks remain active.
- Four CLI tests cover fifteen scenarios and invocations: asset/source downloads
  choose the query-bearing filename across four request forms; invalid path
  values skip only their archive; discarded surrogate fragments permit cache
  publication before snapshot serialization fails; a trailing slash prevents
  publication and later HTTP acquisition after the actual retry schedule.

Bounded findings: **high impact** — reusing ordinary archive fetches could read
the wrong file by removing query text or normalizing a trailing separator.
**High impact** — foreign file authorities previously bypassed the catalogue's
local-host checks. **Medium impact** — raw-byte filesystem errors and unencodable
Python strings have different skip/retry policies. **Medium impact** — fragment
removal occurs before filesystem conversion but does not change the indexed URL.

For L URL code points and B downloaded bytes, parsing, conversion and reading use
O(L + B) work/storage, excluding OS and DNS latency. A native hostname buffer uses
1,024 bytes; address-cache storage is O(A) for A resolved IPv4 addresses. A failing
open performs at most four filesystem/resolver attempts. There is no new deadline
for filesystem/DNS calls and no new payload quota.

Remaining boundaries: Rust's native resolver does not implement Python's complete
IDNA 2003/nameprep codec for Unicode hostnames. Its address ordering and resolver
failure details need separate differential coverage. Read-only probes of the
actual `encodings.idna` codec map `localhost\u3002` to `localhost.` and
`local\u00adhost` to `localhost`, while U+FFFD raises UnicodeEncodeError. Thus a
percent-decoded invalid UTF-8 host also needs the codec's terminal-error policy;
ordinary native resolver failure is insufficient evidence of parity. Native Windows execution,
alternate filesystem codecs, concurrent replacement/resource exhaustion,
custom urllib handlers, exact OSError/outer diagnostics and injected response-read
faults remain unverified. At A82, file-response header checks remained incomplete:
the actual `email.utils.formatdate` accepts epoch seconds -62,135,596,800 and
253,402,300,799 but raises ValueError one second below/above those endpoints.
The actual `mimetypes.guess_type("//[bad]/archive.zip")` raises ValueError during
selector parsing. Those header-stage checks occur after stat and before the
ordinary host check; A83 below adds their validation and failure categories.
Ordinary repository-selected surrogate file URLs still
cross the strict UTF-8 boundary identified under A81. These limits keep QG3 and
QG5 open for full project parity.

The uninterrupted serial all-target run passed 777 tests: 294 unit tests and 483
integration tests across 62 suites, with nine existing opt-in tests ignored and
no failures. All five A73 source-oracle environments were enabled. The 58-test
catalogue integration suite passes in this run. Final formatting, whitespace,
warnings-as-errors Clippy and Windows all-target cross-compilation pass. Logs are
`/tmp/hy-catalogue-file-source-final.log`, `/tmp/hy-catalogue-file-cli.log`,
`/tmp/hy-catalogue-file-full.log`, `/tmp/hy-catalogue-file-clippy-final.log` and
`/tmp/hy-catalogue-file-windows-final.log`. The initial read-oracle comparison
exposed integer-versus-float wait representation in the fixture; the projection
now compares binary64 seconds as the source emits them. The upstream checkout
remained clean at the pinned revision. Full project parity remains open.

### Catalogue file-response timestamp and MIME validation

Under A83, `github/file/headers` owns header-stage validation. The source contract
is `urllib.request.FileHandler.open_local_file`, reached by pinned HCLI's
`_urlopen_with_retry`, `download_source_archive` and `download_release_asset`.
The ordering is stat, modification-date formatting, MIME selector parsing, host
validation and file open. `open_after_stat` preserves that boundary without
coupling filesystem lookup to timestamp conversion.

The timestamp adapter follows CPython 3.13.15's
[`pytime.c`](https://raw.githubusercontent.com/python/cpython/v3.13.15/Python/pytime.c)
and [`_datetimemodule.c`](https://raw.githubusercontent.com/python/cpython/v3.13.15/Modules/_datetimemodule.c).
It rounds the fractional second to microseconds with ties to even, normalizes the
carry/borrow, checks native time_t bounds and invokes `gmtime_r` or `gmtime_s`.
Calendar success is followed by datetime's year range, 1 through 9999. ValueError
equivalents skip an archive; platform calendar OSError equivalents become
retryable URLError; time_t overflow remains terminal. The native consumer needs
the validation outcome rather than an emitted Last-Modified header.

Stat float construction follows
[`posixmodule.c::fill_time`](https://raw.githubusercontent.com/python/cpython/v3.13.15/Modules/posixmodule.c):
floor epoch seconds plus fractional nanoseconds. Native filesystem comparisons
found that the pinned macOS ARM64 Clang build contracts this expression into
fused multiply-add. Separate multiplication/addition differed by one binary64 bit
for a pre-epoch fixture. The adapter uses fused arithmetic on that target and
separate arithmetic elsewhere; other compiler profiles remain an explicit
assumption, not a cross-platform runtime certification.

MIME validation reuses the shared urllib component/authority checks on the
selector independently of the complete request URL. Timestamp validation runs
first. This preserves bracket, IP and NFKC-delimiter errors without forcing
Python-only strings into strict UTF-8 or inventing MIME output that no catalogue
consumer reads. Custom MIME mappings and system-database initialization failures
remain separate from this selector validation.

Evidence:

- 16,747 actual `datetime.fromtimestamp(..., UTC)` comparisons cover both signs,
  every binary64 exponent with four mantissa samples, nonfinite values, 64
  adjacent floats on either side of each datetime year boundary and selected
  half-microsecond ties. Successful values compare complete microsecond datetime
  text; failures compare categories and ValueError/OverflowError text.
- 8,552 actual `mimetypes.guess_type` comparisons cover URL/path syntax and every
  surrogate code point in four selector positions. These compare validation
  acceptance, not host MIME database contents.
- 72 controlled source-handler executions combine nine timestamps, four selectors
  and two hosts. Actual `_urlopen_with_retry` and `FileHandler.open_local_file`
  consume injected stat values and in-memory read responses. Native
  `open_after_stat` opens an owned fixture when validation succeeds. Comparisons
  cover payload, first failing stage/category, attempt count and waits, including
  date failure before MIME/host failure, four calendar-error attempts and terminal
  time_t overflow. The earlier A82 corpus still covers complete request/path/stat
  behavior without this injection.
- 48 owned filesystem fixtures compare the exact binary64 bits of the retained
  `st_mtime`, including pre-epoch nanoseconds and filesystem clamping. Python
  oracles reject filesystem mutation; Rust owns fixture creation and timestamp
  updates.
- One CLI test covers both asset and source archives. This macOS volume clamps
  requested epoch time 253,402,300,800 seconds to 9,223,372,036 seconds plus
  854,775,807 nanoseconds. Both archives therefore exercise valid retained-date
  acquisition and cache publication. On a filesystem that retains year 10000,
  the fixture instead checks skipping before a foreign-host error. The latter
  branch is verified here through the controlled source-handler corpus, not by
  claiming this volume retained an unrepresentable timestamp.

Bounded findings: **medium impact** — header construction can suppress an archive
before host checking, despite the payload itself being readable. **Medium
impact** — calendar conversion failure is retryable while float/time_t overflow
is terminal. **Low impact** — compiler contraction changes stat-float bits; the
cache-expiry and directory-ZIP timestamp consumers, which use separate Duration
conversions, warrant independent boundary comparisons after this header group.

Timestamp conversion uses O(1) work/storage, excluding native calendar internals.
Selector validation uses O(L) work/storage for L code points. The existing maximum
of four retryable open attempts and configured waits of 2, 4 and 8 seconds are
unchanged; no new deadline or byte quota is introduced. Concurrency, alternate
compiler/libc/CRT profiles, MIME initialization failures, exact OS-error text,
IDNA and the A81 shared-fetch surrogate gap remain open under QG3/QG5.

The uninterrupted serial all-target run passed 782 tests: 298 unit tests and 484
integration tests across 62 suites, with nine existing opt-in tests ignored and
no failures. All five A73 source-oracle environments were enabled; all 59
catalogue integration tests passed. Added baseline assertions and the complete
new source corpus passed again after this run. Final formatting, whitespace,
warnings-as-errors Clippy and Windows all-target cross-compilation pass. Logs are
`/tmp/hy-file-headers-source-final.log`, `/tmp/hy-file-headers-cli-final.log`,
`/tmp/hy-file-headers-full.log`, `/tmp/hy-file-headers-final.log`,
`/tmp/hy-file-headers-clippy-final.log` and `/tmp/hy-file-headers-windows-final.log`.
The initial metadata comparison exposed the fused-arithmetic distinction, and
the initial CLI fixture assumed year-10000 retention; the final implementation
and fixture account for the observed behavior above. The upstream checkout
remained clean at the pinned revision. Full project parity remains open.
