...
use crate::util::{json_numbers, python_repr};
...
impl Snapshot {
    pub fn to_json(&self) -> Result<String> {
        ...
        // The source re-parses Pydantic's JSON through CPython before pretty printing.
    ...
    match value {
        ...
        Value::Array(values) => {
            ...
            for (index, value) in values.iter().enumerate() {
                separator(index, depth + 1, output);
                write_value(value, depth + 1, output);
            }
            close(']', values.is_empty(), depth, output);
        }
        Value::Object(values) => {
...
fn separator(index: usize, depth: usize, output: &mut String) {
    if index != 0 {
        output.push(',');
    }
    newline(depth, output);
}

fn close(delimiter: char, empty: bool, depth: usize, output: &mut String) {
    if !empty {
        newline(depth, output);
    }
...
fn newline(depth: usize, output: &mut String) {
    output.push('\n');
    for _ in 0..depth {
        output.push_str("    ");
    }
